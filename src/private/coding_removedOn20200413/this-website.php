<?php
  ...
  define("SITE_SUBSUBTITLE",'Updated on March 27, 2020<br/>Posted on March 23, 2020');
?>
<!doctype html>
<?php
  ...
  <div class="row">
    <div class="col-12 mx-auto col-md-10">
      ...
      <div class="mb-5"><h2 id="key-points" class="anchor">Key points</h2>
        <ul>
          <li><p>I have been building my website using HTML, CSS, JavaScript (including JQuery), PHP and MySQL, with version control powered by GitHub.</p></li>
    ...
    <div class="mb-5"><h2 id="photo-portfolio" class="anchor">Photography portfolio</h2>
      <div class="mb-5"><h3 id="photo-mysql-database" class="anchor">Storing photo attributes using MySQL database</h3>
        ...
        <p>Let me demonstrate how to create such as database. After figuring out the database schema, we can use MySQL to create the database and tables. When we create tables, we need to tell MySQL what the columns are called and what data types they are. Let's start by creating location tables:</p>
        <pre><code class="language-sql">CREATE DATABASE photo_collection;
USE photo_collection;

CREATE TABLE countries
(
  country_id INT unsigned NOT NULL AUTO_INCREMENT, // Unique ID for each country
  name VARCHAR(90) NOT NULL, // Names of countries
  abbreviation VARCHAR(10) NOT NULL, // Abbreviations of countries
  PRIMARY KEY (country_id) // Make country_id the primary key
);
...
?&gt;</code></pre>
    ...
    <div class="mb-5"><h3 id="photo-filtering-javascript" class="anchor">Photo filtering by criteria and updating URL parameters using JavaScript</h3>
      ...
      <pre><code class="language-javascript">const photoCat = document.querySelectorAll('.maincontent .filter-button button');
// Get all category filter button objects from HTML
// In my case, they are under .maincontent .filter-button button

const allPhotos = document.querySelectorAll('.maincontent .gallery .photo-block');
// Get all photos from HTML
// In my case, they are under .maincontent .gallery .photo-block
...
for(let i = 0; i < photoCat.length; ++i){
  ...
  // Trigger this function if the user clicks on category buttons
    ...
    // Call toggleActiveClass to change "active" states of buttons

    toggleimages(photoCat[i].dataset.class); 
    // Call toggleimages to display or hide images depending on their categories

    let param = "?category="; 
    // Construct the first part of the URL parameter
    
    param = param.concat(photoCat[i].dataset.class); 
    // Concatenate the category string with the first part of the URL parameter
...
function toggleimages(dataClass){
  if(dataClass === 'all'){
  // All photos are displayed if category "all" is chosen
      
    for(let i = 0; i&lt;allPhotos.length; ++i){
    // Look through all photos
      allPhotos[i].style.display = 'block'; // Display all photos
    }
  }
    
  else{ // Filter images if any category other than "all" is chosen
    for(let i = 0; i&gt;allPhotos.length; ++i) {
...
for (let i = 0; i < photoCat.length; ++i) {
// Loop through all categories (photoCat)

  if (photoCat[i].dataset.class === category)
    toggleActiveClass(photoCat[i]);
    // Call toggleActiveClass to make the chosen category button "active"
  }
  toggleimages(category);
  // Call toggleimages to display the or hide images depending on the chosen category
}</code></pre>
